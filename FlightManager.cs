...
class FlightManager
    {
        ...
        public bool deleteFlight(int fid)
        {
            int loc = findFlight(fid);
            if (loc == -1) { return false; }
            if (flightList[loc].getNumPassengers() > 0) { return false; }  //  added to avoide deleting the flight with existng bookings
        ...
        public Flight getLastFlight() { return flightList[numFlights - 1]; } 
    }
}
